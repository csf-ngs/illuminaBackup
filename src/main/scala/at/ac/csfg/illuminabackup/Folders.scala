  ...
  */
  ...
  def findSubdirsCommand(): Seq[String] = {
      for{ 
         ...
         "find "+findPath+"/"+d+ " -type f -maxdepth "+subDirsDepth+" ! -name '*.tmp' -print0 "
      }
  }
...
case class RunFolder(path: File) extends Folder { //RunFolder
  ...
  override def includeNames = Seq("RTAConfiguration.xml", "RunParameters.xml" , "RunInfo.xml", "runParameters.xml", "First_Base_Report.htm", "*params") ++ gzipNames.map(_+".gz")
...
class Illumina2Bam(val path: File) extends ResultFolder {//Data/Intensities/Basecalls|Bustard/illumina2bam
    ...
    override def copySubDirs(): Seq[String] = listFiles(path).filter { f => (f.getName.endsWith("fastqc") || f.getName.endsWith("csfstats")) && f.isDirectory }.map(f => f.getName)
